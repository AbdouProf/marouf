...
@php
$accptoffre=\App\Models\offre::where(['demande_id'=>$demande->id,'ostatus'=>'Offre Accepte'])->get();
@endphp
...
<form action="{{ route('demande.update', $demande->id) }}" method="post">
                    ...
                    <div class="content with-padding padding-bottom-10">
                        <div class="row">
                            <div class="col-xl-6 col-md-6 col-sm-6">
                                <div class="utf-submit-field">
                                    ...
                                    <input type="text" class="utf-with-border" placeholder="Ex : N7eb 5kg degla "
                                        name="title" value="{{ $demande->title }}" />
                                    @error('title')
                                ...
                                <div class="utf-submit-field">
                                    ...
                                    <div class="utf-input-with-icon">
                                        <input class="utf-with-border" type="text" placeholder="Enter your address"
                                            name="Dadress" value="{{ $demande->Dadress }}" />
                            ...
                            <div class="col-xl-6 col-md-6 col-sm-6">
                                <div class="utf-submit-field">
                                    <h5>Category</h5>
                                    <select class="selectpicker utf-with-border" data-size="7" title="Choose a category"
                                        id="cat_id" name='cat_id'>
                                        @foreach (\App\Models\Category::where('status', 'active')->get() as $cat)
                                        <option value="{{ $cat->id }}" {{ $cat->id == $demande->cat_id ? 'selected' : ''
                                            }}>{{ $cat->title }}
                                        </option>
                                        @endforeach
                            ...
                            <div class="col-xl-6 col-md-6 col-sm-6">
                                <div class="utf-submit-field">
                                    ...
                                    <select class="selectpicker utf-with-border" id="country-dropdown"
                                        ...
                                        <option value="{{ $country->id }}" {{ $demande->country_id == $country->id ?
                                            'selected' : '' }}>
                                            {{ $country->name }}
                                        </option>
                                        @endforeach
                                        ...
                                        <div class="col-xl-6 col-md-6 col-sm-6">
                                            <div class="bidding-field">
                                                <select class="selectpicker default" name='Date'>
                                                    <option value="Mois" {{ 'Date'=='Mois' ? 'selected' : '' }}>
                                                        Month
                                                    </option>
                                                    <option value="Mois" {{ $demande->Date == 'Mois' ? 'selected' : ''
                                                        }}>Month</option>
                                                    <option value="Jours" {{ $demande->Date == 'Jours' ? 'selected' : ''
                                                        }}>Days</option>
                                                    <option value="Heurs" {{ $demande->Date == 'Heurs' ? 'selected' : ''
                                                        }}>Hours</option>
                                                </select>
                                                @error('Date')
                                                <p class="text-danger">{{ $message }}</p>
                            ...
                            <div class="utf-submit-field">
                                ...
                                <div class="uploadButton margin-top-15 margin-bottom-30">
                                    ...
                                    <label class="uploadButton-button ripple-effect" for="upload" id="lfm"
                                        data-input="thumbnail" data-preview="holder">Upload images (Max 5
                                        images)</label>
                                    <input id="thumbnail" class="form-control" type="text" name="Dimage"
                                        placeholder="Télécharger (jpg, png, pjeg)" value="{{$demande->Dimage}}">
                                    @error('Dimage')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="col-xl-12 col-md-12 col-sm-12">
                            <div class="utf-submit-field">
                                ...
                                <input type="hidden" id="lat" name="lat" value="{{$demande->lat}}" />
                                <input type="hidden" id="lng" name="lng" value="{{$demande->lng}}" />
                                <div id="map"></div>
                            </div>
                        </div>
                        <div class="col-xl-12 col-md-12 col-sm-12">
                            <div class="utf-submit-field">
                                ...
                                <textarea cols="40" rows="2" class="utf-with-border" placeholder="Déscription..."
                                    name="Ddesc" id='Ddesc'> {{$demande->Ddesc}}</textarea>
                                @error('Ddesc')
                                <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
    ...
    <div class="utf-centered-button">
        <button type="submit" class="button utf-ripple-effect-dark utf-button-sliding-icon margin-top-0">Modifier ma
            demande <i class="icon-feather-plus"></i></button>
    </div>
</form>
@else
<h1> Impossible de modifier </h1>
@endif
@endsection
